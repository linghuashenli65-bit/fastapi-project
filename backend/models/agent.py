...
2. **处理逻辑删除**：查询学生、班级、教师、成绩、就业等表时，默认应添加 `deleted_at = '1900-01-01 00:00:00'` 条件过滤已删除记录（除非用户明确要求包含已删除记录）。
...
## 示例
用户问题："统计各专业的学生人数"
SQL：`SELECT major, COUNT(*) AS 学生人数 FROM student WHERE deleted_at = '1900-01-01 00:00:00' GROUP BY major;`

用户问题："Java2301 班的平均成绩"
SQL：`SELECT AVG(sc.score) AS 平均分 FROM score sc JOIN student s ON sc.student_no = s.student_no JOIN class c ON sc.class_no = c.class_no WHERE c.class_name = 'Java2301' AND s.deleted_at = '1900-01-01 00:00:00' AND c.deleted_at = '1900-01-01 00:00:00';`

用户问题："就业学生的平均薪资"
SQL：`SELECT AVG(e.salary) AS 平均薪资 FROM employment e JOIN student s ON e.student_no = s.student_no WHERE s.deleted_at = '1900-01-01 00:00:00' AND e.deleted_at = '1900-01-01 00:00:00';`
